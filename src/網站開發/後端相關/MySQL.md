...
* 資料庫正常化是有效地組織資料庫中的數據的過程。歸一化處理的兩個原因：
    * 消除冗餘數據，例如，存儲了一個以上的相同的數據在同一個表。
    * 確保數據的相關性意義。
* 這兩者都是值得追求的目標，因為它們減少的空間的資料庫消耗的量，並確保數據在邏輯上存儲。規範化由一係列指導方針，幫助指導您創建一個良好的資料庫結構。
* 標準化準則分為正常形態;認為形式的格式或資料庫結構的布局方式。 正常形態的目的是組織資料庫結構，使其符合第一範式，然後第二範式，最後第三範式的規則。
...
* 提交讀（READ COMMITTED）
    ...
    * 可以避免脏读，但還是會有不可重覆讀和幻讀的問題
* 可重複讀（REPEATABLE READ）
    * 未提交的事務的變更不能被其他事務看到，同時一次事務過程中多次讀取同樣記錄的結果是一致的。
    * 可以避免脏读、不可重复读和一部分幻读
    * Mysql 的默認級別，InnoDB 通過 MVVC 解決了幻讀的問題
* 可串行化（SERIALIZABLE）
    * 當兩個事務間存在讀寫衝突時，資料庫通過加鎖強制事務串行執行，解決了前面所說的所有問題（髒讀、不可重複讀、幻讀）。是最高隔離的隔離級別。
    * 可以避免脏读、不可重复读和幻读
