...
所有的 MySQL 類型都可以進行索引，對相關列使用索引是提高 SELECT 查詢性能的最佳途徑。MyISAM 和 InnoDB 都是使用 BTREE 作為索引，MySQL 5 不支持函數索引，但是支持 前綴索引。
...
* 簡介
    * 視圖的英文名稱是 view，它是一種虛擬存在的表。視圖對於用戶來說是透明的，它並不在數據庫中實際存在，視圖是使用數據庫行和列動態組成的表。
...
* 好處
    ...
    * 數據獨立性：一旦視圖的結構 確定了， 可以屏蔽表結構變化對用戶的影響， 數據庫表增加列對視圖沒有影響；具有一定的獨立性。
* 指令
    * 建立 View
        ```sql
...
* InnoDB
    ...
    * 在 5.1 版本後的默認存儲引擎
    * 支持事務操作，具有事務 ACID 隔離特性，默認的隔離級別是可重復讀(repetable-read)、通過MVCC（並發版本控制）來實現的。能夠解決髒讀和不可重復讀的問題。
    * 支持外鍵操作。
    ...
    * 增刪改查性能方面，顆果執行大量的增刪改操作，推薦使用 InnoDB 存儲引擎，它在刪除操作時是對行刪除，不會重建表。
* BDB
* MEMORY
